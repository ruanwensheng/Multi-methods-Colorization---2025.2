...
COCO~2017~\cite{lin2014coco}, and benchmarks it against two representative
pretrained systems spanning the subsequent paradigms: an interactive
CNN~\cite{zhang2017realtime} run in automatic mode and the NoGAN
DeOldify~\cite{antic2019deoldify}. The natural next paradigm in the timeline
would be conditional diffusion, but every well-trained open-source diffusion
colorizer in 2026 is built on Stable~Diffusion~2.1, which Stability~AI has
deprecated upstream---so the diffusion paradigm cannot be evaluated under
identical conditions, and we deliberately exclude it from the comparison
(Sec.~\ref{sec:experiments} explains the exclusion in detail). Our
contributions are:
...
\begin{itemize}
  ...
  \item a three-paradigm comparison under identical conditions, with an honest
        account of where a reported number is misleading (the pretrained head
        mismatch) and why the diffusion paradigm is excluded rather than
        substituted with a degraded model.
\end{itemize}
